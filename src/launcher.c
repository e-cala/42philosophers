...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/07/06 11:56:16 by ecabanas          #+#    #+#             */
/*   Updated: 2023/07/06 11:59:46 by ecabanas         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	death_checker(t_rules *rules, t_philosopher *philo)
{
	...
	while (!(rules->all_ate))
	{
		...
		if (rules->dead)
			break ;
		i = 0;
		while (rules->times_must_eat != -1
			&& rules->num_philos
			&& philo[i].x_ate >= rules->times_must_eat)
			i++;
...
int	launcher(t_rules *rules)
{
	...
	while (i < rules->num_philos)
	{
		if (pthread_create(&(phi[i].thread_id), 
				NULL, p_thread, (void *) &(phi[i])))
			return (1);
